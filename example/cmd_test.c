...
void onmessage(int fd, const unsigned char *msg, uint64_t size, int type)
{
	...
	// Run Command
	bzero(ret_msg, 4096);
	strncpy(ret_msg, (char *)msg, size);
	strncat(ret_msg, " 2>&1", 4096);
	cmd_fp = popen(ret_msg, "r");
	while ((nread = getline(&line, &len, cmd_fp)) != -1) {
		*(line + nread - 1) = '\0';
		ws_sendframe(fd, (char *)line, nread, true, type);
	}
	pclose(cmd_fp);
	free(line);
}
